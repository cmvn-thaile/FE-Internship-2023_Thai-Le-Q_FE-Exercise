...
export const createCart = (cartItems) => {
    ...
    if (cartItems.length > 0) {
        ...
        <td class="cart-table-quantity-group" ><button name=${cartItem.id} id="minus-btn-${cartItem.id}" class="quantity-btn minus-btn">-</button>
        <p id="quantity-${cartItem.id}">${cartItem.quantity}</p>
        <button name=${cartItem.id}  id="plus-btn-${cartItem.id}"  class="quantity-btn plus-btn">+</button></td>
      ${cartItem.discount
                ? `<td id="sub-total-${cartItem.id}" class="sub-total">${calSubTotal(cartItem.price, cartItem.discount, cartItem.quantity)}</td>`
                : `<td id="sub-total-${cartItem.id}" class="sub-total">${calSubTotal(cartItem.price, cartItem.quantity)}</td>`}
...
const renderQuantityBtn = (cartItems) => {
    ...
    for (let btn of quantityBtnArray) {
        btn.addEventListener("click", (e) => {
            e.preventDefault();
            const cartItem = cartItems.filter((item) => item.id === parseInt(btn.name))[0];
            if (cartItem) {
                ...
                if (subTotalCell !== null) {
                    if (cartItem.discount) {
                        subTotalCell.textContent = calSubTotal(cartItem.price, cartItem.quantity, cartItem.discount);
                    }
                    else {
                        subTotalCell.textContent = calSubTotal(cartItem.price, cartItem.quantity);
                    }
                }
            }
            calculateTotal();
...
const renderDeleteBtn = (cartItems) => {
